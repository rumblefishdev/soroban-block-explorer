...
| Indexer fails a CH write                                          | ✅ `indexer-ch-write-failures` (filter on `fields.alarm`, CI-guarded; 🔜 zero-tolerance — any single post-retry failure) + error-rate alarm                                                                                                                     | 2 custom metrics                     | `alarm="ch_write_failure"` events with full error     | partial                                            |
...
| Slack delivery chain itself breaks                                | ➖ OPEN DECISION — **and it broke**: 2026-08-18/19 a topic-policy change revoked CloudWatch's publish right and every alarm action failed for 19 h, found by accident. Mitigations still unchosen (ADR 0054: email co-subscriber / dead-man's-switch / nothing) | —                                    | —                                                     | ➖                                                 |
