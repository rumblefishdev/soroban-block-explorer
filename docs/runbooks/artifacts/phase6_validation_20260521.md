...
| 5 — Horizon compare (smoke + full) | ✓ PASS            | **Full Tier 5 run completed 2026-05-22:** 980 stratified ledgers from the Horizon-retention-valid range (≥ 56,657,428), paginated hash-set compare against `transaction_hash_index`. Result: **980 / 980 PASS, 0 fail, 0.0000 % mismatch.** AC `≤ 0.01 %` exceeded. Earlier smoke result (32 stratified): 0/32 tx-count mismatch, hash 205/205 on probed ledger. Op-count drift documented as Horizon semantic noise (inner Soroban op accounting + fee-bump flattening), not divergence. |
...
- **Operator:** Stanisław Królikiewicz
- **Date:** 2026-05-21 (smoke) / 2026-05-22 (full Tier 5)
- **Phase 5 + Phase 6 status:** **complete**. Full Tier 5 PASS (980/980, 0.0000 % mismatch); ready for go-live pending Snapshot B + PR #199 merge. Deeper per-endpoint parity validation continues under task 0252.
