...
| `snapshot-seed-lp [--artifacts <dir>] [--execute]` | the LP pass `snapshot-seed` deliberately leaves out (task 0374 K4-6): seed the classic LP world the ingest floor hides — `lp_positions` rows for pre-floor holders (`first_deposit_ledger = 0` sentinel, API reads it as null), `liquidity_pools` + one-snapshot stubs for pre-floor pools; ghosts REPORTED only (`ghosts.tsv`, no zeroing — this pass has no RPC-probe evidence yet); versions on each entry's own ledger; summary carries the built-in invariant check `sum(pool_shares_trust_line_count) == decoded live shares` — do not `--execute` on a mismatch | `lp_positions`, `liquidity_pools`, `liquidity_pool_snapshots`, `accounts` — only with `--execute` |
