...
| `prices_admin`     | `prices_write_ddl` | `prices_write` | SELECT, INSERT, ALTER, CREATE TABLE, DROP TABLE, TRUNCATE on `prices.*`; SELECT on `default.*`, `system.parts`, `system.mutations` (inline `<grants>`; task 0567)               | prices-api operator campaigns (history re-ingest, partition repair) — operator-held cert, never a Lambda |
