...
- **Per-module datasource (task 0243)** — read handlers dispatch PG (`sqlx`) or
  ...
  **SAC is a facet, not a type** ([ADR 0051](../../../lore/2-adrs/0051_sac-as-facet-of-classic-credit.md),
  ...
  `code:issuer` via `derive_sac_strkey` — never stored) + `sac_deployed`, the
  canonical `id` of a SAC-wrap is its `CODE-ISSUER`, and the "SAC" view is the
  property filter `filter[sac]=true` (`sac_contract_id != 0`). `/assets/{C…}`
  deep-links resolve either a soroban contract OR a SAC — `fetch_by_contract_id`
  hashes the input `C…` to its `sac_contract_id` surrogate (bloom skip-index) and
  matches the classic/native row.
