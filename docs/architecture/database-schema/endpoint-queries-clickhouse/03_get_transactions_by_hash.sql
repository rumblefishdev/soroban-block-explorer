...
-- Indexes:      transaction_hash_index (ORDER BY (hash_prefix, ledger_sequence)
--                 — the hash's first 8 bytes → candidate ledgers, task 0580).
...
--     via a `transaction_hash_index` seek on the hash's 8-byte prefix — every
--     candidate ledger; `transactions` decides by the full hash (more than one
--     candidate only when two hashes share a prefix). (A `transaction_hash_dict`
...
-- ============================================================================
-- A. Candidate ledgers of the hash (transaction_hash_index prefix seek).
-- ============================================================================
...
WHERE t.hash = $1
  AND intDiv(t.ledger_sequence, 500000)
      IN (SELECT intDiv(ledger_sequence, 500000) FROM transaction_hash_index
          WHERE hash_prefix = reinterpretAsUInt64(substring($1, 1, 8)));
