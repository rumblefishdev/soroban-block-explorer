...
| `transaction_operations`          | `ReplacingMergeTree` (no version, partitioned) | **yes** for the transaction page's operations (one position, cheap); no for the list `operation_types` aggregate, the op-type filter and pool activity — `groupUniqArray`, `LIMIT 1 BY` / `IN` and `max()` collapse an unmerged duplicate (task 0372) |
...
| `soroban_invocations_appearances` | same                                           | not read by the API — still written by the indexer; task 0586 moved every reader to `contract_activity` and drops this table in its PR 3                                                                                                              |
| `contract_activity`               | same                                           | **yes** for the transaction page's invocations and the contract stats (single contract or transaction, cheap); no on the Invocations tab and the `/transactions` contract filter drivers — `LIMIT 1 BY` collapses an unmerged duplicate (task 0586)   |
| `nft_ownership`                   | same                                           | **yes**                                                                                                                                                                                                                                               |
| `liquidity_pool_snapshots`        | same                                           | **yes**                                                                                                                                                                                                                                               |
| `pool_operation_amounts`          | `ReplacingMergeTree` (no version, partitioned) | no — the producer is deterministic (schema header's single-writer argument), so an unmerged duplicate is byte-identical to its twin; the leg pairing in 24 writes it into the same slot                                                               |
