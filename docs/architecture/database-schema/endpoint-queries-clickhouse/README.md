...
| `operations_appearances`          | same                                           | not read by the API — still written by the indexer; task 0372 moved every reader to `transaction_operations` and drops this table in its PR 4                                                                                                         |
...
| `lp_operation_amounts`            | `ReplacingMergeTree` (no version, partitioned) | not read by the API — still written by the indexer; task 0372 moved pool activity to `pool_operation_amounts` and drops this table in its PR 4                                                                                                        |
