...
| `transaction_hash_index`                               | `transaction_hash_index`          | append-only fact | ORDER BY `(hash_prefix, ledger_sequence)` — 8-byte hash prefix, full hash checked in `transactions` (task 0580)                                                                                                                                                                                                                                                                            |
...
The PG `transaction_hash_index` table exists in CH keyed by the first 8
bytes of the hash (task 0580); API reads seek it by that prefix for every
candidate ledger, then read `transactions` in each until the full hash
matches. A `transaction_hash_dict` `DICTIONARY` was
...
- **`balances`** (`holder_id`, `asset_id`, `amount Int128`, `last_updated_ledger`,
  ...
  tokens — see README DECISION 2026-06-29). Spent-to-zero → `amount = 0`;
  **`removed` → `amount = 0` AND `closed_at_ledger` = the removal ledger**
  ([ADR 0055](../../../lore/2-adrs/0055_holding-lifecycle-column-on-balances.md)).
  The lifecycle column exists because those two cases were previously written
  identically, so the read path could not distinguish a live-but-empty holding
  from a closed one and hid both (issue #377). `closed_at_ledger = 0` means
  live — ledger 0 does not exist, so it is a safe sentinel — and the column
  covers all three holding kinds this table unifies (classic trustline, the
  `account_merge` native tombstone, Soroban `ContractData`). `DEFAULT 0` is
  load-bearing: the CH driver rejects inserts client-side when the table carries
  a column the writer struct does not know and that column has no default, so
  the default is what lets the `ALTER` land before the writer deploys.
  Rows are never deleted — mutations cannot keep up with ingest, and RMT
  `is_deleted` cleanup depends on merges that task 0420 measured as not
  reaching a single part here.
...
| `transaction_hash_index`          | `(hash_prefix, ledger_sequence)` (8-byte hash prefix, task 0580)                  | no              |
