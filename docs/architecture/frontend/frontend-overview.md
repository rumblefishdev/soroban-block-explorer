...
| `polling.ts`                    | Per-resource cache policies: `livePolicy` + `midpointPollDelay` (adaptive per-ledger cadence for the home feeds *and* `useNetworkStats` — each fetch aims at the midpoint of the next inter-block gap, anchored on the newest row's close time; server cooperates with `Cache-Control: max-age=0` on these endpoints), `listPolicy` (60s), `detailPolicy` (5min), `searchPolicy` (no cache).                                                                                                                                                                         |
