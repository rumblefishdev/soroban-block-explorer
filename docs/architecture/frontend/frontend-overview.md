...
| `router/routes.ts`              | Re-exports the canonical `routes` table from `@rumblefish/soroban-block-explorer-ui` (`libs/ui/src/routes.ts`) and adds the app-only `NAV_LINKS`. Consumer-facing URL helpers (`routes.home`, `routes.transaction(hash)`, `routes.search(q)`, …) — `<Link to={routes.X}>` / `navigate(routes.X)` instead of hand-templated strings. The single source of truth for URL shapes lives in `libs/ui` (so lib components like `IdentifierDisplay` / `routeForHit` and the app share one table — task 0299); encoding (`encodeURIComponent`) lives there once. |
