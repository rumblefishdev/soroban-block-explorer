...
4. From the decoded XDR, extract the field values to compare:
   ...
   - per-invocation **call tree** (caller_account / caller_contract per task 0183) — **two distinct sources** depending on whether the sub-call requires explicit Soroban auth:
     - **Auth-required sub-calls** (e.g. `transfer` on a SAC: requires `from`'s signature) → reconstructable from `envelope_xdr → operations[i].body.invokeHostFunctionOp.auth[i].root_invocation` walked depth-first. py-stellar-sdk: `inv.function.contract_fn.contract_address` for each `SOROBAN_AUTHORIZED_FUNCTION_TYPE_CONTRACT_FN` node, with parent = the next non-leaf ancestor. **The envelope alone is sufficient — no meta needed.** Use this for verifying caller_contract on routers calling SACs (e.g. `CCQFTEL3.exec → POSTER.transfer` is captured here).
     - **Auth-less sub-calls** (e.g. read-only or implicit-auth from parent contract) → only visible in `result_meta_xdr v4 → diagnostic_events[]` `fn_call`/`fn_return` topics. **Caveat: mainnet stellar-core does not emit fn_call/fn_return by default** — Lobstr-served meta typically has `diagnostic_events: []`. The indexer presumably reads from a Galexie captive-core source with diagnostics enabled. UNVERIFIABLE for these via public sources.
   - **Op-source override semantics** for entry-frame `caller_account`: when an op has an explicit `source_account` override (different from inner-tx source on a fee-bump, or different from tx source on a non-fee-bump), task 0183's call-graph caller is **the op-source, NOT the inner-tx source**. py-stellar-sdk: check `op.source_account is not None` and use that StrKey if so; else fall back to `tx.source_account` (or fee-bump inner). Verifier subagents reading Horizon's flat top-level `source_account` on fee-bumps will report the inner-tx source and miss the op-override — flag as DIVERGENCE only after confirming via envelope decode.
...
- Trusting a verifier subagent's narrative summary of a stellar.expert API response — `WebFetch` post-processes the response with an LLM that can hallucinate flat primitive fields that don't exist in the actual JSON. Observed: `successful: true` reported for a tx that was actually `successful: false`, fabricated `operation_types` like `"Manage Buy Offer"` for txs whose only ops were `path_payment_strict_send`. The stellar.expert `/explorer/public/tx/<hash>` endpoint returns `id, hash, ledger, ts, protocol, body, meta, result` — anything per-tx beyond those four headers requires decoding the base64 XDR (same as Raw XDR subagent 3). Reliable cross-source signal from stellar.expert: `id`, `hash`, `ledger`, `ts`. Anything else: prefer Horizon flat JSON, OR have the subagent decode the XDR explicitly, OR mark UNVERIFIABLE.
- Flagging `application_order` MISMATCH against Horizon's `paging_token` decode — DB `application_order` is **1-based** post task 0172 (matches Horizon convention). Horizon's `paging_token` for a tx encodes `(ledger << 32) | (application_order << 12) | 0`, so `paging_token >> 12` IS the 1-based `application_order` directly (NOT `application_order - 1`). Off-by-one verifier prompts produce systematic false-MISMATCH on every row. Sanity check: the delta between two rows' `paging_token`, divided by 4096, should equal the delta between their DB `application_order`.
- Reading the Horizon `/operations` `parameters` array as a flat list of equal-significance args for `invoke_host_function` ops — the array is positionally typed: `[target_contract_address, function_name_symbol, ...args]`. `parameters[0]` is the contract being invoked; `parameters[2]` (often another address) is typically the first call argument (e.g. the `to` field of a SAC `transfer`), NOT a transitively-invoked contract. A subagent that picks the "first non-`Sym` Address" or "last Address in the array" as the target will misidentify the invoked contract — a cross-check against the envelope XDR's `op.body.invoke_host_function_op.host_function.invoke_contract.contract_address` resolves it (that field IS `parameters[0]`).
