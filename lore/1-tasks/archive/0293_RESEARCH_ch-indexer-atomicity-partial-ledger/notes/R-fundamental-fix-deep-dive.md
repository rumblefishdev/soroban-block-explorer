...
Triggered by user pushback on the Step-5 recommendation: *"check whether we can
fundamentally solve this with commits instead of band-aids."* Adversarial
...
1. **Per-partition staging + atomic `ATTACH/MOVE/EXCHANGE PARTITION`** —
   **REJECT.** Atomic partition publish needs one writer to own a whole 500k CH
   ...
   them *parallel*). Would require re-aligning the backfill unit + S3 sync layer
...
2. **Block-level insert idempotency (`insert_deduplicate` /
   ...
   *is* available on a single node (audit's "replication-only" instinct was
...
- **Killers:** (1) **durability NOT guaranteed by default** — a committed txn can
  come back *partially applied* after a hard crash without fsync (meta-issue
  ...
  hand-rolled `session_id`/373-retry. (6) Gives atomic *visibility*, not dedup —
