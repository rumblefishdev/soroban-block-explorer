...
history:
  ...
  - date: 2026-06-15
    ...
    note: >
      ...
      the fundamental name-side-table per ADR 0050). BATCH:
  ...
  - date: 2026-06-15
    ...
    note: >
      ...
      is the lone late+partial writer on the identity row; ADR 0050 already
...
2. **G5 name-clobber.** _Mechanism CONFIRMED in code, but EMPIRICALLY INERT._
   ...
   NAMES are off-ledger → an ENRICHMENT job (RPC `name()`, ADR 0050/0231 family),
...
- **G5** name-write clobber fix (name-only RMT row must not NULL out
  ...
  table joined at read (ADR 0050 pattern), which eliminates the two-writer
...
- [x] **LIVE inline fix — G1 + G2 + G9 IMPLEMENTED** (2026-06-15, fail-open,
      ...
      **G5 (name-clobber) DEFERRED** — confirmed real bug (PG column-UPDATE vs
      ...
      (ADR 0050). 3rd-Lambda alternative dropped after CTO review. Live-CH
