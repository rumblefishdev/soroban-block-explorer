...
history:
  ...
  - date: '2026-05-11'
    ...
    who: karolkow
    note: |
      Post-merge cleanup pass + feat/0194 merge. Cleanup (commit fe7ed88): dropped dead `NftTokenUriError::NotImplemented` variant (Phase E shipped — stub gone), rewrote stub-era doc blocks in fetcher mod + worker (failure model + cache), replaced 6 stale `nft_metadata` refs with `nft_token_uri` across ADR 0043, schema overview, endpoint-query SQL, drop migration comment, indexer producer doc, and cross-task 0188/0196 bodies (rename-debt from 2026-05-08 module-naming pivot). Collapsed `decode_token_uri_result` `ScVal::Symbol` branch — Symbol XDR limit 32B cannot hold a realistic URI, dead path now rejected via the fallthrough. **Negative caching considered + rejected:** initially added `Ok(None)`-cached permanent fails to skip RPC on repeat broken-NFT detail-page hits; reverted after fresh-eye audit identified three costs: (a) cache-poisoning on flaky IPFS gateway 4xx (cements stale state for cache TTL), (b) lost worker `warn!` observability on DLQ replay / backfill (only first occurrence in 24 h logged), (c) complicates 0196 backfill force-retry (would need cache-clear hook). Production benefit (sub-ms cache-hit savings on rare repeat) didn't justify those trade-offs. Documented as deliberate trade-off in fetcher doc + property-check test (`resolve_propagates_permanent_error_as_err`). Final: 56/56 unit + 1 live-smoke ignored. Merge (commit 7771bc5): pulled feat/0194 with develop's accumulated work (0204 ClickHouse pilot, 0066 frontend TanStack, 0202 nft-events→nft-ownership, 0192 ops-appearances ordering fix, multiple ADRs / wiki); resolved conflicts (icon.rs deletion, lore active/0188+0195 deletions, ADR 0043 matrix combined-source, Cargo.toml `db-clickhouse` member); regenerated api-types per CI gate. 0195 NFT enrichment dispatch logic intact post-merge — `enrichment-worker/main.rs` only changed via 0191's `resolve_or_env()` helper extraction. `cargo check --workspace` + web typecheck both clean.
---
