---
url: 'https://github.com/stellar/stellar-galexie/blob/main/Makefile'
title: 'stellar-galexie Makefile'
fetched_date: 2026-03-25
task_id: '0001'
---

# Makefile - stellar/stellar-galexie
...
Both requested targets are present:

- **`docker-build`** - Builds a Docker image for linux/amd64 with versioning build args, using `docker/Dockerfile`.
- **`docker-test-fake-gcs`** - Depends on `docker-clean`. Sets up a fake GCS server (`fsouza/fake-gcs-server`) on a test Docker network, then runs galexie with `scan-and-fill --start 1000 --end 2000` against it using `docker/config.test.toml`.
