---
url: 'https://raw.githubusercontent.com/soroswap/core/refs/heads/main/README.md'
title: 'Soroswap.Finance Core Smart Contracts'
...
- SoroswapFactory: `CA4HEQTL2WPEUYKYKCDOHCDNIV4QHNJ7EL4J4NQ6VADP7SYHVRYZ7AW2`
  ...
  - [See in StellarChain](https://stellarchain.io/contracts/3872426bd59e4a61585086e3886d457903b53f22f89e361ea806ffcb07ac719f)
...
### 4. (For production): Publish those .json files and serve them using Vercel
...
1. Run the Stellar Quickstart and the `@esteblock/soroban-preview` Docker containers. Here you can choose to use a `standalone` or `futurenet` instance:
...
This will take all standalone deployments available in `.soroban` (local) and will do 1) Mint test tokens, 2) Provide Liquidity 3) Swap 4) Remove Liquidity.
...
The following example showcase a local manual testing. Make sure you have deployed the contracts to the network and saved it locally.

We provide code for manual testing of the contracts using `typescript`.

To run all the transactions we could do on soroswap protocol just run (inside soroban-preview image):

```bash
# Usage:
# yarn test <network> <public>
# network: Type of the network to configure (standalone, futurenet, testnet, testnet-public)
# public: OPTIONAL: If you type `public` as a 2nd argument, you will run test against the addresses in the public folder
...
To run tests against contracts addresses in the public `public` folder, do:
...
where `network` is the network (it could be mainnet, testnet or standalone) to set the trustlines and `folder` is the folder where the token-list is located (public or .soroban).
