...
1. **No node-id provided in Wave 6 dispatch.** Per `figma-use` skill conventions, every `mcp__figma__get_screenshot` call requires `(fileKey, nodeId)`. Picking node-ids from archived tasks without confirming they match the _current_ live FE state means audit findings could be against an outdated design.
...
## What Wave 6 _can_ report from cross-sub-phase observation
