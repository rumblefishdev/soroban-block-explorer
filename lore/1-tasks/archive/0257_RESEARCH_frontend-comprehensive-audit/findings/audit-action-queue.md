...
> - **PoolParticipants share-% — NOW GENUINELY FIXED** (card 7.3 DONE). Under design*parity R2 this was **VERIFIED ILLUSORY** (`formatAmount(*,2)`= minDecimals, no rounding — see line above). 0272 replaced it with`formatPercent(Number(row.share_percentage))` (`PoolParticipants.tsx:54-55`, real `.toFixed(2)` cap). Code-verified real this pass. **Supersedes the 2026-05-29 ILLUSORY verdict.**
>
> **Still TODO (0272 did NOT touch):** F-DP-3 / card 11.3 (raw z-index → named scale — `d184457f` even _added_ a new raw `zIndex:3` on TopNav); F-DP-4 / card 11.4 (OperationFlowTree collapse — verify still data-blocked).
>
...
- **Severity / Class:** 🟠 D
- **Pre-launch:** MUST
- **STATUS:** SKIP (2026-06-08, explicit user decision — total skip, not deferred)
...
- **Pre-launch:** MUST
- **STATUS:** DONE (2026-06-06, live-verified) — `/contracts` is now a REAL list page (0275): `web/src/pages/ContractsListPage.tsx` + `useContractsList.ts` + `ContractsTable.tsx` + `ContractsFilters.tsx`, route `router/index.tsx:103` (no longer PageStub). Live with real data 2026-06-06: 20 rows, real contract hrefs, h1 "Contracts", filters. `/accounts` half DONE earlier (0274/design_parity R2). Both halves of F-A-5 Gap 1 closed.
...
- **Severity / Class:** 🟡 C
- **Pre-launch:** SHOULD
- **STATUS:** CLOSED (2026-06-08, lore-0280 fresh-eyes) — both genuine pieces DONE, rest SKIP. **PageStub deleted ✓** (dead code, 0 consumers, → `.trash/`, commit `87b4860c`). **F-X-1 pool coupling DONE ✓** (extracted `FeePill`/`PoolAssetPair`/`helpers` → `web/src/pages/pool-shared/`; bidirectional liquidity-pools↔pool-detail tangle now 0↔0; typecheck + 83 tests green). **Everything else SKIPPED** (total skip, not deferred).
- **Fresh-eyes verdict (2026-06-08, lore-0280) — most of this card is cosmetic, SKIP:**
  - **Hoist `detail/` (11 files) → libs/ui — SKIP.** All 11 are pure presentational (no app-coupling) so technically lib-eligible, BUT `libs/ui` has exactly ONE consumer (web) — `grep` confirms libs/ui never imports `detail/`. So the move yields **zero real reuse**, only theoretical "purity", at the cost of **39 import-site rewrites**. High churn, no functional gain, no user impact. The folder works.
  - **`web/src/search/` → `pages/search/` — SKIP (card OVERSIMPLIFIES — search is MIXED, verified 2026-06-08).** Consumer grep: `GlobalSearchBar` → `AppShell`, `directRouteFor` → `AppShell` + `HomeHero` + `SearchResultsPage` = genuinely **cross-cutting** (top-level placement defensible). BUT `SearchResultsView` + `useSearchResults` → only `SearchResultsPage`, and `SearchResultRow`/`SearchResultsTabs` are internal = **page-specific** (could move under `pages/search/`). So the card's blanket "move all to pages/" is wrong for the cross-cutting half; a clean fix would SPLIT the folder (cross-cutting stays shared/top-level, page-specific moves). More nuance than the card states, still cosmetic — not worth the diff.
  - **`web/src/utils/` (1-file) + page-root helpers (`cursorParams`/`url`) — SKIP.** Single-file folder + helpers-next-to-pages hurt nobody; moving live, working files = churn-for-aesthetics, zero value.
  - **PageStub delete — DONE** (the one genuine win: real dead code).
  - **F-X-1 pool coupling — DONE ✓** (the one genuine smell: extracted shared `FeePill` + `PoolAssetPair` + `helpers` → new `web/src/pages/pool-shared/`; `liquidity-pools/` and `pool-detail/` now import one-directionally from it — bidirectional reach eliminated, 0↔0. 4 `git mv` + 6 import updates across 5 files; pure refactor, typecheck + 83 web tests green).
...
- **Code-verified 2026-06-08 (branch `research/0257`, task 0280 ROI sweep) — PageStub GATE NOW CLEARED:** card 1.3 shipped the real `ContractsListPage` (0275) + `AccountsListPage` (0274), so `grep -rn PageStub web/src/` returns **ZERO live consumers** — `PageStub.tsx` is a true dead orphan again. **F-AH-1 flips back TRUE (deletable)**; the "gated behind card 1.3" condition is satisfied — deletion (via `mv .trash/`) can now proceed in this card. Also: `web/src/pages/detail/` has grown to **11 files** (DataListCard, FeeCell, KpiCell, KpiStripSkeleton, ListPageSkeleton, PageBreadcrumb, PageHeader, SectionCard, SummaryRow, SummarySkeleton, TableSectionSkeleton) — hoist target materially larger than the original 3-file estimate; re-scope the `libs/ui` hoist accordingly. `web/src/search/` + `web/src/utils/` still present (structural debt unchanged).
...
- [ ] F-AH-1 — `web/src/pages/PageStub.tsx` dead orphan — **RE-CONFIRMED TRUE 2026-06-08: card 1.3 shipped real Accounts (0274) + Contracts (0275) pages, PageStub now has ZERO consumers again → deletable (`mv .trash/`). The `06ab34cc`/R2 "FALSE — 2 live consumers" note is itself now superseded.**
...
- **SKIP rationale (2026-06-08, lore-0280):** Prototyped on the 0280 branch
  (flag on + 6 fix-up sites, `nx typecheck` green) then **reverted and
  skipped**. Two reasons: (1) the natural fix-up at "provably-present" index
  sites is a silent `?? ''` / `?.` fallback = **soft-failing** (swallows a
  violated invariant and renders wrong/empty data) — the opposite of the
  loud hard-fail the value proposition implies; doing it "right" needs
  `invariant`/throw at every such site. (2) It flips a **repo-wide**
  `tsconfig.base.json` strict flag = a team-convention change that shouldn't
  ride in on an audit-closing task without buy-in. Not a pre-launch blocker
  (SHOULD). If revived later it needs: throw-based handling (not `??`
  fallbacks) + team sign-off.
...
- **Severity / Class:** 🟠 D
- **Pre-launch:** SHOULD
- **STATUS:** PARTIAL (2026-06-08, lore-0280) — branded types DROPPED; only `isAssetId` added. See decision note.
...
- [ ] F-AQ-4 — Zero branded / nominal types for ID strings — **SKIP/DROPPED** (2026-06-08). Branded types were prototyped (`branded.ts` + 5 type-guards) then **removed**: a brand has no teeth without threading the branded type into the consumers that should _demand_ it (`routes.account(id: AccountId)`, hook signatures), and that threading **cascades** for little gain on a runtime-dispatch router. Foundation-without-payoff = dead scaffolding; dropped rather than left. Revisit only if/when full enforcement-threading is wanted (team decision).
- [x] C-5 — Missing `isAssetId` / `isNftId` validator — **isAssetId DONE** (polymorphic `boolean` validator: `native` | contract StrKey | `CODE-ISSUER`, with tests — 16 ui tests green). **isNftId intentionally NOT added** — NFT identity is composite `(contract_id, token_id)` per ADR 0030 / 0264 Phase 8a; no single-string NFT id to validate.

**Notes:** Pairs with future router-param-validation work (0067 deferred AC).
**2026-06-08 decision (lore-0280):** kept ONLY `isAssetId`; branded types dropped (see F-AQ-4). **`isAssetId` now WIRED into `AssetDetailPage` param-guard** (commit `503a9485`) — AssetDetailPage was the lone detail page not pre-validating its URL param; the 5 siblings already skip-fetch + NotFound via `is*Id`. Wiring was blocked while assets routed by numeric `surrogate_id` (`isAssetId` rejects it), but **task 0243 `route_token` was merged into feat/0280 (merge `311c6929`)** so the asset id is now always canonical (`native` | StrKey | `CODE-ISSUER`) — surrogate gone — and the canonical-only guard is correct. Tests use canonical CODE-ISSUER params + a malformed-id case; web 84 green. C-5 fully closed.
...
- **Severity / Class:** 🟡 D
- **Pre-launch:** NICE
- **STATUS:** SKIP (2026-06-09, lore-0280, user decision) — type-nicety, no current bug. Prototyped (assertNever helper + adopted in 4 genuinely-exhaustive switches: useSearchResults, usePoolChart, HighlightedJson, OperationFlowTree) then reverted — the 3 return-typed switches are already protected by `noImplicitReturns`, and the one implicit-fallback (OperationFlowTree) is low-risk. The 2 remaining listed sites are N/A anyway: `humanizeOp.ts` is intentionally partial over op-types (`break` + generic fallback, `type_name: string`), `validators.ts` has no switch (card stale). Not worth the cross-package export surface for the marginal guard.
...
- [ ] F-AQ-3 — 4 switches, 3 exhaustive, 1 implicit-fallback; no `assertNever` — **SKIP (lore-0280): type-nicety, no bug; `noImplicitReturns` already guards the return-typed ones.**
...
- **Severity / Class:** 🟠 D
- **Pre-launch:** SHOULD
- **STATUS:** DEFER post-launch (2026-06-09, user decision) — perf-only, zero user-facing correctness; the woff2 font swap already cut ~1MB off load, so the >500KB Vite warning is no longer pressing. `manualChunks` vendor split + lazy `PoolCharts` + `rollup-plugin-visualizer` = a ~1d optimisation worth doing post-launch, not a launch blocker.
...
- **Severity / Class:** 🟡 D
- **Pre-launch:** NICE
- **STATUS:** SKIP (2026-06-09, lore-0280) — **valuable half already DONE, remainder not worth it.** Code-verified: `ContractDetailPage` already uses `useTabUrlState` → the Interface/Invocations/Events tabs (real data) ARE URL-state. The only open part is `PoolCharts.tsx` metric/period (`useState`), but the LP chart renders a placeholder until task 0199 ships the series (metrics are null) — so URL-state for an empty chart has near-zero value now. Skip; revisit only if 0199 lands pre-launch and the chart carries real data.
...
- [x] F-E-7 — No URL state for tabs — **Contract tabs half DONE (`useTabUrlState` in ContractDetailPage); LP chart half SKIP (placeholder until 0199).**
- [ ] F-EX-2 — Pool chart metric/period in useState — **SKIP (lore-0280): chart is a 0199-blocked placeholder; URL-state for an empty chart is near-zero value.**
...
- **Severity / Class:** 🟠 D
- **Pre-launch:** SHOULD
- **STATUS:** DONE (2026-06-09, lore-0280) — 0066 triple-drift fixed + completed/archived; 0063 same-class drift fixed + closed. Sweep of every `status: active` FE task (`grep` for body `## Status: Backlog` / `Not started` against `active/`) returns **zero remaining drift** — those two were the only cases. The Phase-3 walker script (automation) is the one un-done sub-item, left as optional nice-to-have (the manual sweep is exhausted and the drift is cleared).
...
- **Severity / Class:** 🟠 D
- **Pre-launch:** SHOULD
- **STATUS:** CHECKLIST (2026-06-09, lore-0280) — the un-spawned future-work items are tracked **as a checkable checklist below** instead of 23 task files (user decision). The queue is the single source of truth; **tick each `[ ]` off as it ships.** Card clears only when the checklist clears (Tier-5 are explicit-skip candidates).

**Rationale.** Wave 1 archaeology found 25 of 28 Future Work items from archived FE tasks have NO spawned backlog task. Rather than spawn 23 files (churn + drift risk), they live here as a checklist.

**Future-work checklist (from `00-archaeology.md`, currency-checked 2026-06-09):**

_Already resolved since the audit:_

- [x] ~~0059 live stats wiring (TopNav)~~ — DONE (TopNav serves real stats, live-verified).
- [x] ~~0059 responsive/hamburger nav~~ — DONE (card 11.5).
- [x] ~~0075 contracts list + nav~~ — DONE (0275 / card 1.3).

_Tier 1 — real value, doable:_

- [x] **0069 op_type enum from backend** — ✅ DONE 2026-06-09 (lore-0280). Backend registers `domain::OperationType` in OpenAPI `components(schemas(...))` → codegen emits a named `OperationType` union in `@rumblefish/api-types`; FE `operationTypes.ts` now keys `DISPLAY_LABELS: Record<OperationType, string>` exhaustively against it (compile error on backend drift). Hand-maintained 27-entry mirror gone. **(dedupe with card 6.3 — same item, F-Z-2.)**
- [ ] **0075 `wasm_interface_metadata` JSONB shape doc** — FE reverse-engineers the shape; backend should document/surface it. **(dedupe with card 6.3.)**
- [ ] **0062 `IdentifierDisplay` `<Link>` vs `<a>` audit** — raw `<a>` causes full-page reload instead of SPA nav. Largely handled by the lore-0272 sweep (7 links converted); verify no stragglers remain.

_Tier 2 — tech-debt / refactor (no user impact):_

- [ ] **0072 hoist `Button` + `formatFee` + 2-line timestamp → `libs/ui`** — kills `pages/ledgers` → `pages/transactions` coupling (same class as F-X-1).
- [ ] **0069 `libs/ui` error/empty divergence from Figma DS** — fix state visuals at source.
- [ ] **0062 validators → `libs/domain`** — architectural; requires creating `libs/domain`.

_Tier 3 — design sign-off:_

- [ ] **0069 operation-pill colour** — confirm palette with design.

_Tier 4 — gated on other work (can't start yet):_

- [ ] **0077 Playwright CLI regression for LP pages** — gated on 0226 (now shipped → unblocked; folds into card 8.1).
- [ ] **0068 functional table sorting** — gated on backend exposing a sort param.
- [ ] **0075 events count for tab pill** · **0077 per-leg `icon_url`** — backend extensions.
- [ ] **0077 Tx Amount on PoolTransactions** — gated on 0247 (RESEARCH).
- [ ] **0077 chart series wiring** — gated on 0199 (blocked-on-oracle).
- [ ] **0075 SAC SEP-41 interface stub** — niche feature.

_Tier 5 — skippable NICE (LP polish, post-launch — explicit skip-candidates):_

- [ ] 0077 pool-id paste search
- [ ] 0077 list sort options
- [ ] 0077 tx-type filter
- [ ] 0077 row distinction for stale pools
- [ ] 0077 LP a11y review
- [ ] 0077 LP mobile QA

**Findings closed (sub-checklist):**

- [x] A3 — 25/28 Future Work items un-spawned — **RESOLVED: catalogued inline above (not spawned, per user decision).**
- [x] AC-13 — Each unchecked AC tracked — **via the inventory above.**
- [x] Q-7 — Forward-link 0254 ↔ 0257 testing baseline — **8.1 owns the testing residual; cross-linked.**

**Notes:** Cards 1.3 + 7.2 + 8.3 + 11.5 + 0275 already closed several (struck through above). Tier 1 op_type/wasm items DEDUPE with card 6.3 — do them there, not twice.
...
- **Severity / Class:** 🟠 E
- **Pre-launch:** SHOULD
- **STATUS:** PARTIAL (1 of 4 done, lore-0280) — **item 4 (op-type OpenAPI enum / F-Z-2) DONE 2026-06-09**: `OperationType` registered in OpenAPI components → codegen owns it, FE mirror killed. **3 remain OPEN:** item 1 CORS infra question (C-17), item 2 `wasm_interface_metadata` schema (F-AQ-7), item 3 `results_meta_xdr` codegen drift (F-AQ-8).
...
4. **Operation type enum codegen from OpenAPI** — ✅ **DONE 2026-06-09 (lore-0280).** `domain::OperationType` registered in OpenAPI `components(schemas(...))` (`crates/api/src/openapi/mod.rs`) → `@hey-api/openapi-ts` emits a named `OperationType` union in `@rumblefish/api-types`; FE `transactions/operationTypes.ts` keys `DISPLAY_LABELS: Record<OperationType, string>` exhaustively against it. Resolves 0069 Future Work + F-Z-2. Zero wire change (response op-type fields stay `string`; the enum is a standalone documented component). api-types regenerated.
...
- [x] F-Z-2 — Operation type enum hand-typed in FE — **DONE 2026-06-09 (lore-0280): `OperationType` now generated from OpenAPI; FE label map exhaustive over it.**
- [x] Z-1 Spot 5 — Same as F-Z-2 — **DONE (see F-Z-2).**
...
- **STATUS:** PARTIAL — F-0272S-1 / F-0272S-2 / F-0272S-3 / F-0272S-4 DONE (verified post-merge `afd36e77` 2026-06-03); **F-0272S-1 RESOLVED by 0274** — real `GET /v1/accounts` endpoint (`crates/api/src/accounts/handlers.rs:69`) + non-mock FE hook (`useAccountsList.ts` now `listAccountsOptions`, mock gone) → row-click 404 fixed. F-0272S-5 (tx-type multi-select), F-0272S-6 (shared search semantics) still open (NICE).

**Rationale.** 0272's closure session ran a list-page filter/sort/search code audit (no fixes applied) and surfaced concrete dispositions. lore-0276 then resolved the FE-only + small-backend items (S-2 LP partial search, S-3 dead-sort cleanup, S-4 search hints); S-1 / S-5 / S-6 remain open. Captured here as the authoritative record so nothing is lost. F-0272S-1 owned by spawned active task 0274. Source: `archive/0272_FEATURE_audit-0257-closing.md` §Session findings + §Future Work + top-of-file 0272 merge-note block.
...
- [x] **F-0272S-2 — LP vs assets search inconsistency (real bug).** (✅ DONE in lore-0276) Assets `filter[code]` = partial substring `a.asset_code ILIKE '%'||$1||'%'` (`assets/queries.rs:132`); LP `filter[asset_code]` = **EXACT** `UPPER(lp.asset_a_code)=$9 OR UPPER(lp.asset_b_code)=$9` (`liquidity_pools/queries.rs:340`). That is why LP needed the whole code, assets did not. **Correction:** prior type-stub comment said LP was "partial" — wrong, SQL was equality. Note LP searches by **asset code**, not pool id. **Resolved in lore-0276:** both legs now `ILIKE '%'||$9||'%'` (`liquidity_pools/queries.rs:352-353`) + api-types regen.
- [x] **F-0272S-3 — Dead sort UIs (assets total-supply + ledgers sequence).** (✅ DONE in lore-0276: removed dead assets sort arrow, ledgers sequence sorting wired to backend)
- [x] **F-0272S-4 — Silent no-op searches (transactions + NFT).** (✅ DONE in lore-0276 via input validation warnings) Transactions (`TransactionsListPage.tsx:40`): only fires a filter when input is a full G-account (`→ filter[source_account]`) or full C-contract (`→ filter[contract_id]`); tx-hash / partial / anything-else → **no filter sent, silent no-op** (no tx-hash search, no partial). NFT (`NftsListPage.tsx`): collection = exact equality on `collection_name`; contract requires valid C-strkey; partial → empty. By design but reads as broken. UX fix = placeholder + empty-state hints. **Done in lore-0276** (input-validation warning state on tx filter via `DebouncedField` + isAccountId/isContractId; search-hint placeholders).
...
**Notes:** **Resolved in lore-0276:** F-0272S-2 (LP `asset_code` ILIKE partial), F-0272S-3 (dead assets sort arrow removed + account-tx sort revived via `order` param), F-0272S-4 (tx/NFT search validation + hints). **Still open — suggested action split for remaining items:** backend-medium/backlog = `/v1/accounts` list endpoint (F-0272S-1, owned 0274 — kills not-found bug + makes accounts sort real); backend+FE = op-type multi-select (F-0272S-5); architectural = shared search-semantics contract (F-0272S-6). Verify 0274 actually covers F-0272S-1 when picked up.
...
- **Severity / Class:** 🟡 C
- **Pre-launch:** NICE
- **STATUS:** SKIP (2026-06-09, lore-0280, user decision) — cosmetic micro-batch, not worth a coordinated landing pre-launch. Triaged this session: **F-W6-E2-2 typo FIXED** ("All operations type" → "All operation types", `TransactionFilters.tsx`); **F-W6-E5-1 already DONE/stale** (`LedgerNav` renders a disabled pill at a null prev/next boundary); the regressed **F-AK-1/AK-2 (hex, z-index) are owned + DONE under cards 11.2 / 11.3**. The ~24 remaining sub-findings are subjective design-judgment cosmetics (memo "—" semantics, tab descriptions, search-box dedup, "?" fallback richness, status-badge icon cue, GPU transitions, copy nits) — SKIP pre-launch; cherry-pick post-launch with a designer if desired.
...
- [x] F-W6-E2-2 — "All operations type" typo — **FIXED 2026-06-09 (lore-0280): `TransactionFilters.tsx` MenuItem + comment now "All operation types".**
...
- [x] F-W6-E5-1 — Prev/Next ledger button no disabled state for boundary — **DONE/stale: `LedgerNav` renders a disabled pill when prev/next sequence is null (verified 2026-06-09).**
...
- **Severity / Class:** 🟡 C
- **Pre-launch:** NICE
- **STATUS:** DONE (stale finding — verified 2026-06-09, lore-0280). Heading hierarchy is already correct: NFT name is `component="h1"` (`NftDetailPage.tsx:149`) and all three section headers (Details / Traits / Transfer history) render `component="h2"` via the shared `TableSectionHeader` (`libs/ui/src/table/TableSectionHeader.tsx`). The finding predates the `TableSectionHeader` h2 adoption. No code change.
...
- [x] F-W6-E11-1 — NFT detail has h1 but no h2/h3 — **DONE/stale: section headers are h2 via `TableSectionHeader`; NFT name is h1 (verified 2026-06-09).**
- [x] F-W6-F-1 — Same finding (recap) — **DONE/stale (see F-W6-E11-1).**
...
- **Severity / Class:** 🟢 B
- **Pre-launch:** NICE
- **STATUS:** SKIP / non-issue (2026-06-09, lore-0280 currency check) — stale premise. Both consumers (`AppShell.tsx:83` header stats + `home/ChainOverview.tsx:22`) now call the **same `useNetworkStats` hook** → identical query key → TanStack already dedupes to a single request. The "two requests / separate query key" the rationale assumes no longer exists. Nothing to do.
...
- **STATUS:** DONE (lore-0276, verified post-merge `afd36e77` 2026-06-03) — LOADSKEL-1 (home), LOADSKEL-2 (lists), **and LOADSKEL-3 (all 7 detail routes `*DetailSkeleton.tsx`) ALL shipped** + wired as route-specific Suspense fallbacks (`router/index.tsx:38/45/52/67/82/97/109/117/124/132/139`). Supersedes the earlier "LOADSKEL-3 deferred NICE" note. **Residual minor → F-RR-34 (🟢):** `/contracts` LIST route still uses generic `<DetailSkeleton/>` (`router/index.tsx:103`) instead of `<ListPageSkeleton/>` like the other 5 lists — cold-load shows detail-shaped skeleton on a list page. Cosmetic one-liner. Architectural note: per-route skeletons fix only the cold/first-load flash; the broader UX win (keep prior page on in-app nav) = React Router `lazy` + top progress bar, logged as a separate option. **NEW finding 2026-06-01.**
...
- [x] **F-W6-LOADSKEL-3** ✅ DONE (lore-0276, verified `afd36e77`) — all 7 per-route `*DetailSkeleton.tsx` (account/asset/contract/ledger/nft/pool/transaction) shipped + wired as route-specific Suspense fallbacks. Exceeded the "deferred NICE" plan. (`router/index.tsx`, `pages/**/­*DetailSkeleton.tsx`, `pages/detail/{SummarySkeleton,TableSectionSkeleton,KpiStripSkeleton}.tsx`)
- [x] **F-RR-34** ✅ FIXED (`2f83a39e` on research/0257) — `/contracts` list route now passes `<ListPageSkeleton/>` matching the other 5 lists (`router/index.tsx:102-107`). One-liner.
...
- **Type:** REFACTOR
- **Effort:** ~1h residual (was ~3h; coverage + envelope-preserve already shipped)
- **Severity / Class:** 🟡 A
- **Pre-launch:** SHOULD
- **STATUS:** DONE (2026-06-09, lore-0280) — sensible parts; reporter deferred to a provider decision.

- **2026-06-09 decision (lore-0280):** shipped the one genuinely-valuable residual — **F-AF-4 `[object Object]` guard** in `client.ts` (only adopt a _string_ envelope `message`; a non-string would `String()`-coerce to "[object Object]" as user-facing text). **`extractErrorCode` + a console `reportError`/global handler were prototyped then DROPPED** (fresh-eyes): `extractErrorCode` had zero consumers (nothing branches on the code; the human message is already on `error.message`), and a **console-only** reporter is dev-redundant (the browser already logs uncaught errors + unhandled rejections) and prod-useless (no one watches a prod browser console) — value only materialises once it forwards to a real backend. So **F-AE-7 (global reporter) is DEFERRED** until a Sentry/DataDog provider is chosen; it can't be meaningfully done console-only. Card core (boundary 7/7 + envelope `.body` + F-AF-4 guard) complete; F-AF-2 left as-is (mutating the caught Error to attach `.status` is the standard interceptor pattern).

- **Code-verified 2026-06-08 (branch `research/0257`, task 0280 ROI sweep) — card was STALE:**
  - `SectionErrorBoundary` now wraps **all 7 detail pages** (Account/Nft/Contract/LiquidityPool/Ledger/Asset/Transaction) + HomePage — NOT "2/7" as the original rationale claimed. F-AE-3 DONE.
  - `client.ts` interceptor no longer fully flattens: it **preserves the raw envelope as `.body`** (`Object.assign(new Error(message), { status, body: error })`) — downstream CAN reach `error.body.code`. F-AF-1 / F-AE-4 substantially mitigated (ergonomic `extractErrorCode` helper still missing).
  - **Residual (keeps card PARTIAL):** typed `extractErrorCode(error): string | null` helper (ergonomic `.code` access over `.body`); global error reporter decision (F-AE-7); console shape-probe (F-AE-6); F-AF-2 `Object.assign(error)` mutation guard; F-AF-4 `[object Object]` defensive guard.

**Rationale.** `client.ts` error interceptor flattens typed `ErrorEnvelope` (ADR 0008 `{code, message}`) into a vanilla `Error` — downstream consumers cannot discriminate on `error.code`. ~~`SectionErrorBoundary` only wraps 2/7 detail pages.~~ **[STALE: now 7/7 — see code-verified note above.]** No global error reporter (Sentry / DataDog). Silent shape mismatches have no console signal.

**Scope.** Add typed `extractErrorCode(error: unknown): string | null` helper next to `client.ts`. ~~Wrap all 7 detail pages in `SectionErrorBoundary`.~~ **[DONE — 7/7 wrapped.]** Spawn `XXXX_FEATURE_frontend-error-reporting` task or wire a minimal Sentry/console reporter behind env var. Add runtime shape probe (per F-AE-6 / F-D-1 root-cause prevention).

**Findings closed (sub-checklist):**

- [~] F-AF-1 — Error interceptor swallows raw envelope shape — **PARTIAL: raw envelope now preserved as `.body` (2026-06-08 verify); ergonomic `extractErrorCode` helper still missing**
- [x] F-AE-3 — SectionErrorBoundary inconsistent coverage — **DONE: 7/7 detail pages wrapped (2026-06-08 verify)**
- [~] F-AE-4 — Error interceptor flattens typed envelope (cross-cite F-AF-1) — **PARTIAL: same as F-AF-1, `.body` preserved**
...
- **Severity / Class:** 🟢 D
- **Pre-launch:** NICE
- **STATUS:** TODO (2026-06-09 currency: **favicon DONE** — `web/public/favicon.svg` exists, no longer 404; residual = color-mode localStorage key naming + `<time dateTime>` semantic element + em-dash/ellipsis convention doc)
...
- **Severity / Class:** 🟠 A
- **Pre-launch:** SHOULD
- **STATUS:** SKIP (2026-06-09, user decision — total skip) — the LP-oracle / price-source decision is owned by **task 0199** (LP analytics: TVL + volume + fee_revenue). Not 0280's to carry; closed here, lives on 0199.
...
- **Severity / Class:** 🟢 D
- **Pre-launch:** NICE
- **STATUS:** SKIP (2026-06-09, lore-0280) — **INVALID FINDING.** Code-verified:
  both `linked={false}` sites render the pool detail page's OWN id, not a
  cross-entity reference: `PoolDetailHeader.tsx:39` (`value={poolId}`, subtitle
  under the H1 pair) and `PoolSummary.tsx:92` (`value={pool.pool_id}` in the
  summary row). Both are on `/liquidity-pools/:id`. `linked={false}` is standard
  **self-link suppression** (an entity does not link to its own detail page) —
  NOT a "fix-by-hide" of a broken href. Flipping to `linked={true}` would create
  a self-link (page X → page X), an anti-pattern. The card's premise is wrong; no
  action.

**Rationale (original — refuted).** 0251 B1 set `linked={false}` on pool-id header to hide the bug rather than fix the href routing. Future junior may reintroduce the broken link. Now that 0264 has shipped strkey canonical, root-cause fix should be trivial — re-enable the link with correct strkey routing. **[Refuted: the false flag is self-link suppression on the page's own id, not a broken-href workaround.]**
...
- **Severity / Class:** 🟠 C
- **Pre-launch:** SHOULD
- **STATUS:** DEFER post-launch (2026-06-08, user decision) — still flat (only dead `defaultExpanded?` prop, no useState/Collapse/chevron); verify remains blocked on (a) soroban/multi-op data + (b) designer Figma sign-off. Not a launch blocker; revisit post-launch.
...
- **Severity / Class:** 🟠 C
- **Pre-launch:** SHOULD
- **STATUS:** DEFER post-launch (2026-06-09, user decision) — WCAG 2.1 AA target-size; 105/106 interactive elements <44px @375. Real a11y/mobile gap but not a launch blocker; pick up post-launch with the responsive batch.
...
| F-RR-2      | ✅                        | `home/HeroSearch.tsx:100`                                                                                                             | "CTRL + K" hint pill — no global Cmd/Ctrl+K handler exists anywhere; advertises dead shortcut                                                                                                                                                                                                                                                                                                                                                                 | ✅ DONE (lore-0276, `28465276`) — wired hotkey and kept pill                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| F-RR-3      | ✅                        | `home/LatestTransactions.tsx:53-68`, `LatestLedgers.tsx:51-66`                                                                        | Footer "{rows.length} latest records" renders in loading+error (shows "0 latest records" under skeleton/error)                                                                                                                                                                                                                                                                                                                                                | ✅ DONE (lore-0276, `3724e0d3`) — gated footer on success branch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| F-RR-6      | ✅                        | `transaction-detail/sections/OperationPicker.tsx:89`                                                                                  | Heading hardcoded "Choose payment" but lists ALL op types (Figma copy bleed)                                                                                                                                                                                                                                                                                                                                                                                  | ✅ DONE (lore-0276, `bcc440b5`) — heading → "Choose operation"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| F-RR-7      | ✅                        | `transaction-detail/advanced/EventsSection.tsx:80-90`                                                                                 | `event.contract_id` rendered as plain `truncateMiddle` text, not a contract link — missed by the 2026-05-28 7-site identifier sweep                                                                                                                                                                                                                                                                                                                           | ✅ DONE (lore-0276, `3724e0d3`) — wrapped in `IdentifierDisplay`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| F-RR-14     | ✅                        | `ledgers/LedgerSummary.tsx:23-68`                                                                                                     | Summary key/value reimplemented via local `Cell`/`Row` instead of shared `SummaryRow`/`SectionCard`; also no semantic `<h2>`                                                                                                                                                                                                                                                                                                                                  | ✅ DONE (lore-0276, `fd18f15e`) — LedgerSummary semantic h2 via TableSectionHeader                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| F-RR-17     | ✅                        | `pool-detail/PoolCharts.tsx:126,155,200-251`                                                                                          | `isError` destructured but never surfaced → fetch error renders as "No activity / try longer range" empty state, no retry; every sibling section shows `QueryErrorState`                                                                                                                                                                                                                                                                                      | ✅ DONE (lore-0276, `3724e0d3`) — surfaced `isError` + retry via `QueryErrorState`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| F-RR-18     | ✅                        | `liquidity-pools/FeePill.tsx:24`                                                                                                      | `formatPercent(Number(raw), raw)` — 2nd arg is the non-finite fallback string; NaN fee renders raw `0.300000…` instead of em-dash                                                                                                                                                                                                                                                                                                                             | ✅ DONE (lore-0276) — dropped 2nd arg → NaN renders default `—`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| F-RR-25     | ✅                        | `search/SearchResultsView.tsx:69-135`                                                                                                 | `/search` reimplements error + 3 empty states INLINE — no retry button, swallows error classification (rate-limit/transient/generic→one msg). The one true SM-1 straggler (= F-W4R-1)                                                                                                                                                                                                                                                                         | ✅ DONE (lore-0276, `3724e0d3`) — routed search error through `QueryErrorState` + exposed refetch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| F-RR-33     | ✅                        | `libs/ui/src/states/errors/NotFoundState.tsx` (+ consumers)                                                                           | **Live @375:** entity-404 NotFoundState echoes the full unbroken strkey in "We couldn't find anything matching this `G…`" → element width 403px on 364px viewport → **page horizontal overflow (sw 383 > cw 364)**. Affects every entity-404 with a long id (account/contract/pool). Found live on `/accounts/GSETYMKA…` (a mock-list id → 404).                                                                                                              | ✅ DONE (lore-0276) — fixed at render site `EmptyState.tsx` (meta): `overflowWrap:anywhere`+`maxWidth:100%`; strkey wraps, verified live @335/375 (sw=vw, no h-scroll)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **F-RR-35** | ✅ **FIXED** (`c214f95e`) | `libs/ui/src/identifiers/routes.ts` (`routeForHit`)                                                                                   | **✅ FIXED + live-verified 2026-06-06** — `routeForHit` now uses `surrogate_id` ONLY for `asset`; account/contract/pool route by `identifier` (strkey). Added `routeForHit.test.ts` (6 cases, green). Live: search account/contract strkey → `/accounts                                                                                                                                                                                                       | /contracts/<strkey>`→ real detail loads (was 404). _(orig:_ **🚨 Search redirect → "not found" for accounts / contracts / liquidity-pools. Found LIVE 2026-06-05 with real backend data.**`routeForHit`does`hit.surrogate_id != null ? String(surrogate_id) : identifier`— blindly prefers the numeric`surrogate_id` for ALL non-NFT hits. The code comment admits the assumption was written for the **assets** route (`/v1/assets/:id`accepts numeric), but it's applied to every entity. Search now returns`surrogate_id`for account+contract hits too (0274/0243), so: search`GBHH24…strkey`→ redirect`/accounts/15805579`→`invalid_account_id` ("account_id must be a 56-char StrKey starting with G") → **"Account not found"**. Same for contracts (`surrogate_id:2154201`) + pools. Assets (numeric route) + ledgers (sequence==identifier) + NFTs (composite, handled separately) are FINE. **Regression** exposed by surrogate_id now populated on strkey-entity hits. Repro: `/v1/search?q=<account-strkey>`returns`identifier`(strkey)+`surrogate_id`(numeric); `routeForHit` picks numeric; account detail rejects numeric. | **FIX:** in `routeForHit`, use `surrogate_id` only for numeric-addressed entities (asset); for account/contract/liquidity_pool use `identifier` (strkey). Pre-launch **SHOULD** (search is core; valid search → 404). |

### F-RR-36 cluster — Surrogate-id (DB row-id) anti-pattern in user-facing URLs (architectural, 2026-06-06)

Senior deep-dive (triggered by the F-RR-35 hotfix smell — the asset `surrogate_id` special-case). Verdict: **using the DB autoincrement PK (`surrogate_id`) in any user-facing URL is an anti-pattern; entities must route by their blockchain-canonical id** (StrKey / hash / sequence / asset code-issuer / contract_id). F-RR-35 fixed account/contract/pool but left assets on the surrogate. Root cause is backend-side (search projects a non-canonical asset identifier). All claims verified in code.

> **✅ CLUSTER RESOLVED via task 0243 (Design A: `route_token`) — 2026-06-06, on `feat/0243-accounts-contracts-assets-ch-read-path`; PENDING MERGE into research/0257.** A fresh-eyes + adversarial review on 0243 **rejected** the "uniform `identifier`" prescription below (would destroy the asset/nft display headline + can't remove the irreducible nft 2-component special-case). Shipped instead: `identifier` stays the display headline; a separate `route_token: Option<String>` carries the canonical `/assets/:id` token (contract StrKey | `CODE-ISSUER` | `native`, asset-only, `skip_serializing_if`); `routeForHit` routes `route_token ?? identifier`. **My interim F-RR-35 hotfix (asset surrogate special-case) will be SUPERSEDED by 0243's route_token on the next develop→research/0257 merge — reconcile then.** Per-finding below.

- [x] **F-RR-36** (🟡 SHOULD) ✅ DONE (0243) — `AssetItem.id` is now the canonical String token (`i32→String`), so `AssetsTable` `routes.asset(row.id)` is canonical; no `row.id` surrogate leak.
- [x] **F-RR-37** (🟠 SHOULD, backend) ✅ DONE-differently (0243) — `asset_hits` CTE projects `route_token` (canonical) ALONGSIDE display `identifier` (asset code), not instead of. api-types regenerated.
- [x] **F-RR-38** (🟡 SHOULD, FE) ⚠️ SUPERSEDED (0243) — original "delete special-case → uniform identifier" rejected on review; shipped `routeForHit` = `route_token ?? identifier`. No further FE change. (My F-RR-35 hotfix superseded on merge.)
- [x] **F-RR-39** (🟡 SHOULD, backend) ✅ DONE (0243) — `parse_asset_id` has a `native` branch; `/assets/native` routable.
- [x] **F-RR-40** (🟢 NICE, backend) ✅ DONE (0243) — `surrogate_id` removed from ALL search buckets; `route_token` is asset-only.

**Original prescription (SUPERSEDED — kept for trail):** F-RR-37 (backend canonical asset hit) → F-RR-36 (AssetsTable canonical route) → F-RR-38 (drop FE special-case) → F-RR-39 (native keyword) → F-RR-40 (trim leak). 0243's Design A (`route_token`) replaced the F-RR-37/38 "uniform identifier" approach after adversarial review. Net result identical: zero DB surrogate ids in any user-facing URL.

**Canonical-id ground truth** (verified): account=`G…` strkey · contract=`C…` strkey · pool=`L…` strkey · ledger=sequence · tx=hash · nft=composite(contract_id,token_id) · **asset=code+issuer (classic) / contract_id (SAC) / native (XLM)** — the only entity with no single strkey, but all forms are canonical + endpoint-accepted (≠ needing surrogate).

**API detail-endpoint surrogate audit (2026-06-06, all by-id + sub-resource endpoints swept).** Blast radius CONFIRMED CONTAINED to assets. Every other detail endpoint is **canonical-only** on the wire:

| Endpoint                                                              | Accepts                                                                       | Class                                                                                              |
| --------------------------------------------------------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| `/v1/accounts/{id}`(+`/transactions`)                                 | `G…` strkey only                                                              | Canonical                                                                                          |
| `/v1/contracts/{id}`(+`/interface`,`/invocations`,`/events`)          | `C…` strkey only                                                              | Canonical                                                                                          |
| `/v1/liquidity-pools/{id}`(+`/participants`,`/transactions`,`/chart`) | `L…` SEP-23 strkey → 32-byte hash (ADR 0024 DB-native, not autoincrement)     | Canonical                                                                                          |
| `/v1/nfts/{contract}/{token}`(+`/transfers`)                          | composite `(C-strkey, token_id)`                                              | Canonical                                                                                          |
| `/v1/ledgers/{sequence}`                                              | `u32` sequence                                                                | Canonical                                                                                          |
| `/v1/transactions/{hash}`                                             | 64-hex hash                                                                   | Canonical                                                                                          |
| **`/v1/assets/{id}`(+`/transactions`)**                               | **numeric `assets.id` (autoincrement, tried FIRST) OR `C…` OR `code-issuer`** | **⚠️ Mixed — surrogate is a first-class path form** (`assets/handlers.rs:48-49`, `queries.rs:149`) |

→ Assets are the **sole** place a raw DB row-id is an accepted (and priority-first) path input. Contracts/NFTs/pools resolve to internal i32/i64 surrogates too, but those are NEVER on the wire. So F-RR-36..40 = the complete fix scope; no other endpoint needs touching.
...
- ~~**F-AK-2 / F-DP-3** — raw z-index literals~~ → **RESOLVED** by card **11.3 DONE (2026-06-02)**; this 2026-06-01 "still-open" note is superseded (card newer). Appendix rows flipped RESOLVED in the 2026-06-06 reconciliation.
...
- **F-0272S-1** — ✅ **RESOLVED (0274, merge `afd36e77`)** — real `GET /v1/accounts` endpoint + non-mock FE hook shipped; row-click 404 gone. (was: accounts list = mock → 404.)

**SHOULD (14)** — pre-launch quality bar

- **F-RR-17** ✅ **DONE** (lore-0276, `3724e0d3`) — surfaced PoolCharts fetch error + retry via QueryErrorState
- **F-RR-33** ✅ **DONE** (lore-0276) — EmptyState meta `overflowWrap:anywhere`+`maxWidth:100%`; strkey wraps, verified live @335/375.
- **F-RR-25** ✅ **DONE** (lore-0276, `3724e0d3`) — routed search error through QueryErrorState with retry
- **F-RR-2** ✅ **DONE** (lore-0276, `28465276`) — wired hotkey and kept pill.
- **F-RR-3** ✅ **DONE** (lore-0276, `3724e0d3`) — gated home footer latest record counts on success branch
- **F-RR-6** ✅ **DONE** (lore-0276, `bcc440b5`) — "Choose payment" → "Choose operation" heading on all-op picker.
- **F-RR-7** ✅ **DONE** (lore-0276, `3724e0d3`) — wrapped event.contract_id in IdentifierDisplay link
...
- **F-RR-14** ✅ **DONE** (lore-0276, `fd18f15e`) — LedgerSummary semantic h2 via TableSectionHeader.
- **F-W6-LOADSKEL-1** ✅ **DONE** (lore-0276) — HomeSkeleton route fallback; phase-1 ≈ phase-2, verified live.
- **F-0272S-3** ✅ **DONE** (lore-0276) — removed dead assets sort arrow, ledgers sequence sorting wired to backend.
- **F-0272S-4** ✅ **DONE** (lore-0276) — silent no-op search resolved via helper text input validation warnings.
- **F-0272S-2** ✅ **DONE** (lore-0276) — LP exact vs assets partial search resolved via ILIKE matching.
...
- **F-0272S-6** (no shared search semantics) — **RULED NICE / POST-LAUNCH (2026-06-06).** Was flagged "decision is SHOULD-grade, gates several SHOULD fixes" — but those gated fixes (F-0272S-2/3/4) ALL shipped in round-2 (0276), so the gating concern is moot. Remaining scope = _documenting_ a search-semantics contract = post-launch consistency work, NOT a pre-launch blocker. Confirmed out of 0280 pre-launch scope. (Resolves the only NICE-vs-SHOULD ambiguity flagged in the 2026-06-06 verification sweep.)
...
> **POST-MERGE UPDATE `afd36e77` (2026-06-03) — round-2 closure landed.** The pre-launch batch is essentially DONE. Merge of **0276** (round-2 closing, PR #247) + **0274** (accounts endpoint) + **0275** (contracts list) closed **the MUST (F-0272S-1) + 13 of 14 SHOULD**: F-RR-2/3/6/7/14/17/18/25/33, F-W6-LOADSKEL-1/2/**3** (LOADSKEL-3 exceeded plan — all 7 detail skeletons), F-0272S-2/3/4. **Only open SHOULD = F-RR-21** (search a11y — explicit SKIP in 0276 scope; public-launch a11y gap remains). New residual: **F-RR-34** (🟢 contracts-list route uses DetailSkeleton not ListPageSkeleton). Remaining queue = F-RR-21 + ~20 NICE + 3 POST. Verified skeptically against merged prod code (not commit messages).
>
> **DECISIONS + LIVE VERIFY 2026-06-05:**
>
> - **F-RR-21 → SKIP (user decision 2026-06-05).** Search-a11y deferred post-launch (consistent with the original 0276 SKIP). Public-launch a11y gap consciously accepted for now; revisit post-launch.
> - **F-RR-34 → FIXED** (`2f83a39e`, on research/0257 — fix lives on the audit branch, no separate branch).
> - **Live re-verify (fresh `:4201`):** first pass had local backend/data DOWN; **then brought backend UP** (`cargo lambda watch` :9000 on PG default-datasource — PG has 180k accounts / 23k ledgers / 8.6M tx / 12.9k assets / 12k pools). **Structural wins confirmed live:** accounts mock GONE (F-0272S-1), Cmd/Ctrl+K hint (F-RR-2), `/contracts` REAL list page (card 1.3 + 0275), per-route skeletons. **F-RR-33 fix confirmed** (account-detail @375 no overflow). **768px tablet sweep:** all routes clean.
> - **LIVE DATA-WALK 2026-06-05 (backend UP, real PG data) — completes merge-iteration step 6:** ✅ home (real KPIs Accounts 180,612 / Contracts 11,926, latest-tx rows, live indicator), ✅ `/accounts` 20 real rows, ✅ **account detail loads real data — `GBHH24…` h1+balance+summary, NOT "not found"** (F-0272S-1 fully fixed live — the old mock-404 is dead), ✅ `/contracts` 20 real rows + real hrefs, ✅ `/liquidity-pools` 20 rows fee 0.30%, ✅ pool detail (h1 asset-pair, charts, participants, fee capped). **🚨 ONE NEW BUG found (only visible with real search data): F-RR-35** — global search redirect → "not found" for account/contract/pool (see F-RR-35 row). This is exactly the class of bug the data-walk exists to catch.
...
| A4                                             | 1             | 🟡      | C 8.1                   | RESOLVED    | 0226 test infra — shipped PR #225 (ab170804): Vitest + Testing Library + 132 tests across 17 files (card 8.1)                                                                                                                                                    |
...
| F-AQ-3                                         | 1             | 🟡      | C 3.3                   | SKIP        | Switch exhaustiveness + assertNever — SKIP, type-nicety no bug (lore-0280)                                                                                                                                                                                       |
| F-AQ-4                                         | 1             | 🟠      | C 3.2                   | SKIP        | Branded ID types dropped — no teeth without cascade-threading (lore-0280)                                                                                                                                                                                        |
...
| C-5                                            | 2             | 🟡      | C 3.2                   | PARTIAL     | isAssetId DONE; isNftId N/A (composite id, ADR 0030) — lore-0280                                                                                                                                                                                                 |
...
| J-2                                            | 2             | 🟡      | C 2.1                   | RESOLVED    | 10 toLocaleString sites bypass formatter → formatAmount/formatInteger, 0 left (= F-J-2; card 2.1)                                                                                                                                                                |
...
| J-4                                            | 2             | 🟠      | C 2.1                   | RESOLVED    | STROOPS_PER_XLM single site → single STROOPS_PER_XLM_BIGINT shared util (= F-J-4; card 2.1)                                                                                                                                                                      |
| J-5                                            | 2             | 🟡      | C 2.1                   | TODO        | Timestamp depth inconsistency                                                                                                                                                                                                                                    |
| J-6                                            | 2             | 🟢      | C 8.6                   | TODO        | No `<time>` element (cross-cite C-8)                                                                                                                                                                                                                             |
| J-7                                            | 2             | 🟠      | C 2.1                   | RESOLVED    | Truncation re-impls (6 sites) → canonical truncateMiddle (= F-J-7; card 2.1, `c57f7c4d`)                                                                                                                                                                         |
...
| DM-1                                           | 2             | 🟠      | C 7.2                   | RESOLVED    | Footer "All systems operational" hardcoded — resolved-as-absent (string no longer in codebase; footer carries no health badge) (card 7.2)                                                                                                                        |
| DM-2                                           | 2             | 🟢      | C 7.2                   | TODO        | No /health probe                                                                                                                                                                                                                                                 |
| DN-1                                           | 2             | 🟠      | C 1.2                   | TODO        | No build SHA in UI                                                                                                                                                                                                                                               |
| DN-2                                           | 2             | 🟡      | C 1.2                   | TODO        | No vite define block                                                                                                                                                                                                                                             |
| CA-1                                           | 2             | 🟠      | C 1.1                   | RESOLVED    | Footer Terms/Privacy/Cookies dead spans — removed entirely per user 2026-05-29 (no legal content) (card 1.1)                                                                                                                                                     |
| CA-2                                           | 2             | 🟠      | C 1.1                   | RESOLVED    | Footer Resources wired (GitHub / Stellar docs / Soroban docs / Stellar dashboard) (card 1.1)                                                                                                                                                                     |
| CA-3                                           | 2             | 🟢      | C 1.1                   | RESOLVED    | FooterLink adds target=\_blank rel=noopener noreferrer for external links (card 1.1)                                                                                                                                                                             |
...
| F-K-7                                          | 3             | 🟡      | C 5.4                   | RESOLVED    | E3 tx-detail ledger sequence link → type=ledger (TransactionSummary.tsx:151-152) (card 5.4)                                                                                                                                                                      |
| F-K-8                                          | 3             | 🟡      | C 5.4                   | RESOLVED    | Soroban call tree destination account → IdentifierDisplay in OperationFlowTree.tsx:151 (card 5.4)                                                                                                                                                                |
...
| F-E-7                                          | 3             | 🟡      | C 5.2                   | PARTIAL     | Contract tabs URL-state DONE; LP chart half SKIP (0199-blocked placeholder) — lore-0280                                                                                                                                                                          |
...
| F-Z-2                                          | 5             | 🟢      | C 6.3                   | DONE        | Op-type enum now generated from OpenAPI (`OperationType` union); FE map exhaustive (lore-0280, 2026-06-09)                                                                                                                                                       |
...
| F-AB-3                                         | 5             | 🟢      | C 10.3                  | SKIP        | INVALID — `linked=false` is self-link suppression on pool's own id, not fix-by-hide (lore-0280)                                                                                                                                                                  |
...
| F-EX-2                                         | 5 sweep       | 🟢      | C 5.2                   | SKIP        | Pool chart metric/period useState — SKIP, chart is 0199-blocked placeholder (lore-0280)                                                                                                                                                                          |
...
| F-W6-V-1                                       | 6             | 🟠      | C 7.2                   | RESOLVED    | All live pills now have freshness via useLiveStatus (live/delayed/offline/unknown); DM-1 resolved-as-absent (card 7.2)                                                                                                                                           |
| F-W6-V-2                                       | 6             | 🟡      | C 7.2                   | TODO        | Backfill doesn't disable LIVE                                                                                                                                                                                                                                    |
| F-W6-V-3                                       | 6             | 🟢      | C 7.2                   | RESOLVED    | Latest-ledger polling works (informational) — confirmed (card 7.2)                                                                                                                                                                                               |
...
| F-W6-AK-2                                      | 6             | 🟢      | C 7.1 / C 11.3          | RESOLVED    | Z-index raw 0/1 no scale + `06ab34cc` raw `zIndex: 2` regression — folded into typed zIndex scale on createExplorerTheme; all raw literals migrated (card 11.3, DONE 2026-06-02)                                                                                 |
...
| F-W6-RESPONSIVE-3                              | 6             | 🟠      | C 11.5                  | RESOLVED    | Hamburger nav <md — slim right Drawer in SecondaryNav, live-verified 2026-06-01 (card 11.5)                                                                                                                                                                      |
| F-W6-RESPONSIVE-4                              | 6             | 🟠      | C 11.6                  | TODO        | still failing live; 105/106 elements <44px @375 → card 11.6                                                                                                                                                                                                      |
| F-W6-RESPONSIVE-5                              | 6             | 🟡      | C 11.7                  | RESOLVED    | search page overflow <660px — RESOLVED live 2026-05-29 (scrollWidth 364 ≤ 375; category row scrolls within overflow-x:auto container). Residual per-card reflow = optional NICE (card 11.7)                                                                      |
| F-W6-NOTFOUND-1                                | 6             | 🟡      | C 5.1                   | RESOLVED    | NotFound missing h1 on 4 of 5 detail — h1 WONTFIX per user (tag-only change, no visual diff); `<main>` landmark half DONE (card 5.1)                                                                                                                             |
| F-W6-NOTFOUND-2                                | 6             | 🟡      | C 5.3                   | RESOLVED    | Sub-section queries fire on parent 404 — DONE, 0 sub-reqs live (render-gate on parent.data != null), live-verified 2026-06-01 (card 5.3)                                                                                                                         |
| F-W6-E0-1                                      | 6             | 🟠      | C 1.1                   | RESOLVED    | Footer dead spans (recap) — zero dead spans left (legal removed, Resources wired, Explorer nav onClick) (card 1.1)                                                                                                                                               |
| F-W6-E0-2                                      | 6             | 🟠      | C 7.2                   | RESOLVED    | Footer hardcoded operational (recap of DM-1) — resolved-as-absent, footer carries no health badge (card 7.2)                                                                                                                                                     |
| F-W6-E0-3                                      | 6             | 🟡      | C 11.5                  | RESOLVED    | No hamburger at mobile (recap) — slim right Drawer hamburger shipped in SecondaryNav, live-verified 2026-06-01 (card 11.5)                                                                                                                                       |
| F-W6-E0-4                                      | 6             | 🟡      | C 7.1                   | TODO        | Header search placeholder 4 vs hint 5                                                                                                                                                                                                                            |
| F-W6-E0-5                                      | 6             | 🟢      | C 7.6                   | TODO        | Header polling duplicates home                                                                                                                                                                                                                                   |
| F-W6-E1-1                                      | 6             | 🟡      | C 7.2                   | RESOLVED    | LIVE badge freshness on Latest tx/Ledgers — DELAYED/OFFLINE/CONNECTING when stale/errored/loading (card 7.2)                                                                                                                                                     |
...
| F-W6-E1-4                                      | 6             | 🟡      | C 5.4                   | RESOLVED    | Home ledger hash not a link → IdentifierDisplay type=ledger (home/LedgersTable.tsx:36-42) (card 5.4)                                                                                                                                                             |
...
| F-W6-E6-1                                      | 6             | 🟡      | C 5.3                   | RESOLVED    | Sub-section queries fire on 404 (account) — 0 sub-reqs live (render-gate), live-verified 2026-06-01 (card 5.3)                                                                                                                                                   |
| F-W6-E6-2                                      | 6             | 🟢      | C 5.1                   | RESOLVED    | NotFound no h1 (account) — h1 WONTFIX per user (tag-only, no visual diff) (card 5.1)                                                                                                                                                                             |
...
| F-W6-E9-1                                      | 6             | 🟡      | C 5.3                   | RESOLVED    | Sub-section queries fire on 404 (contract) — 0 sub-reqs live (render-gate), live-verified 2026-06-01 (card 5.3)                                                                                                                                                  |
| F-W6-E9-2                                      | 6             | 🟢      | C 7.1                   | TODO        | Invocations+Events no empty-state message                                                                                                                                                                                                                        |
| F-W6-E9-3                                      | 6             | 🟡      | C 5.1                   | RESOLVED    | NotFound h1 inconsistent (contract) — h1 WONTFIX per user (tag-only, no visual diff) (card 5.1)                                                                                                                                                                  |
...
| F-W6-E10-3                                     | 6             | 🟡      | C 5.4                   | RESOLVED    | NFT row Contract ID plain text → IdentifierDisplay type=contract (NftsTable.tsx:45) (card 5.4)                                                                                                                                                                   |
...
| F-W6-E11-3                                     | 6             | 🟡      | C 5.4                   | RESOLVED    | NFT Contract ID in Details plain text → IdentifierWithCopy type=contract (NftSummary.tsx:76) (card 5.4)                                                                                                                                                          |
| F-W6-E12-1                                     | 6             | 🟡      | C 7.1                   | TODO        | Pool ID truncation twice per row                                                                                                                                                                                                                                 |
| F-W6-E12-2                                     | 6             | 🟢      | C 7.1                   | TODO        | "Any TVL" filter looks like loading                                                                                                                                                                                                                              |
| F-W6-E13-1                                     | 6             | 🟠      | C 7.3                   | RESOLVED    | Pool participants share % full precision — FIXED 2026-06-01: formatPercent(Number(share_percentage)) caps at 2dp (card 7.3)                                                                                                                                      |
| F-W6-E13-2                                     | 6             | 🟢      | C 5.1                   | RESOLVED    | Pool NotFound no h1 — h1 WONTFIX per user (tag-only, no visual diff) (card 5.1)                                                                                                                                                                                  |
...
| F-DP-3                                         | design_parity | 🟠      | C 11.3                  | RESOLVED    | Raw `zIndex: 2` added across shell (AppShell/TopNav/SecondaryNav/Footer) — typed custom zIndex scale declared on createExplorerTheme + all raw literals migrated (card 11.3, DONE 2026-06-02)                                                                    |
...
| Z-1 Spot 5                                     | 5             | 🟢      | C 6.3                   | DONE        | Op-type enum hand-typed (cross-cite F-Z-2) — DONE lore-0280                                                                                                                                                                                                      |
...
| 0251 B1                                        | arch          | 🟢      | C 10.3                  | SKIP        | INVALID — linked=false = self-link suppression (own pool id), not fix-by-hide (lore-0280)                                                                                                                                                                        |
| 0059 Future Work (live stats)                  | arch          | —       | —                       | RESOLVED    | Wired via 0066 (TopNav still shows MOCK_STATS — re-verify in C 7.6 / 8.6)                                                                                                                                                                                        |
| 0059 Future Work (responsive nav)              | arch          | —       | C 8.3 / C 11.5          | RESOLVED    | Hamburger menu — slim right Drawer shipped in SecondaryNav, live-verified 2026-06-01 (card 11.5)                                                                                                                                                                 |
| 0061 FW (libs/ui vitest)                       | arch          | —       | C 8.1                   | RESOLVED    | 0226 promote — 0226 landed PR #225 (Vitest infra + baseline) (card 8.1)                                                                                                                                                                                          |
...
| 0069 FW (OpenAPI op_type enum backend)         | arch          | —       | C 6.3                   | DONE        | OperationType registered in OpenAPI components; codegen owns it (lore-0280, 2026-06-09)                                                                                                                                                                          |
