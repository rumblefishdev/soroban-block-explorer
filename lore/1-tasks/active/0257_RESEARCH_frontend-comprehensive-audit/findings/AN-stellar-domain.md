...
- **Net-negative vs prior baseline.** The original F-AN-6 verdict was "single-environment config, no toggle (acceptable, document it)". A toggle that *looks* functional but does nothing is worse for users than no toggle. This flips F-AN-6 from a 🟢 doc nit to a 🟠 misleading-affordance regression.
