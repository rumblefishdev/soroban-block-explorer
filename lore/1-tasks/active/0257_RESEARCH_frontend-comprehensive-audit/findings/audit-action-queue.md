...
- [x] **F-0272S-2 — LP vs assets search inconsistency (real bug).** (✅ DONE in lore-0276) Assets `filter[code]` = partial substring `a.asset_code ILIKE '%'||$1||'%'` (`assets/queries.rs:132`); LP `filter[asset_code]` = **EXACT** `UPPER(lp.asset_a_code)=$9 OR UPPER(lp.asset_b_code)=$9` (`liquidity_pools/queries.rs:340`). That is why LP needs the whole code, assets does not. **Correction:** prior type-stub comment said LP was "partial" — wrong, SQL is equality. Note LP searches by **asset code**, not pool id. Fix → LP `ILIKE '%'||..||'%'` on both legs (1 line + regen). **Owned by 0274** (backend) — cross-ref card 6.3.
