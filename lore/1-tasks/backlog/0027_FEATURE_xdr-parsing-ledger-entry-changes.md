...
**Current state:** Not started. Depends on task 0024 for parsed LedgerCloseMeta data. Database schema tasks 0019 (tokens, accounts) and 0020 (NFTs, pools, snapshots) define the target tables.
...
This is the final parsing stage in the pipeline (after 0024, 0025, 0026). It consumes the parsed LedgerCloseMeta and NFT-related events emitted by task 0026, then upserts derived-state tables.

All derived-state writes use watermark-based upserts (task 0028) to ensure backfill data does not overwrite newer live-derived state.
...
- `metadata`: initialize JSONB metadata object. Interface data may be populated by task 0026's contract interface extraction.

Write to `soroban_contracts` table. Upsert on `contract_id`: deployment fields on first insert, metadata updated when interface extraction completes (from task 0026).
...
Populate the `nfts` table using a combination of LedgerEntryChanges and NFT-related events emitted by task 0026:
...
- soroban_contracts upsert must handle the case where task 0026's interface extraction runs before or after the deployment extraction in this task. Both paths must converge on the same contract row.
