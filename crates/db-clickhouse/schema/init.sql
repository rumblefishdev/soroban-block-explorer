...
-- transaction_operations: one row per identity-folded group of a
-- transaction's operations, located by the transaction POSITION (task 0372,
-- ADR 0059) — keyed `(ledger_sequence, application_order, operation_index)`.
-- Replaces `operations_appearances`, which located it by the `transaction_id`
-- hash surrogate (33.3 GiB, ratio 1.57 on prod, 2026-09-24); that table is
-- written until its readers move and lives in `transitional.sql` meanwhile.
--
...
-- 0-based one. Replaces `lp_operation_amounts` (keyed by the `transaction_id`
-- surrogate and the 1-based operation index), which is written until pool
-- activity moves and lives in `transitional.sql` meanwhile.
--
