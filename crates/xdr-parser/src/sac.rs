...
/// Forward-derive verdict for `soroban_contracts.is_sac` (task 0218).
///
/// Produced by [`derive_sac_overrides_from_assets`] for every classic
/// asset (Native + ClassicCredit) we observe in `ExtractedAsset` slices.
/// Each override pins one `contract_id` (the deterministic SAC address
/// derived from the asset's `ContractIdPreimage::Asset(_)` envelope) to
/// its underlying `SacAssetIdentity` — used by both the PG-side persist
/// step (UPDATE `soroban_contracts SET is_sac=TRUE, contract_type=Token`)
/// and the CH-side stage (re-insert a corrected `ContractRow` so RMT
/// collapses to the SAC-flagged version).
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SacOverride {
    /// Deterministic SAC contract address (`C...` StrKey).
    pub contract_id: String,
    /// Underlying classic asset identity that maps to this SAC.
    pub identity: SacAssetIdentity,
}

/// Build an XDR `Asset::CreditAlphanum*` from a `(code, issuer)` pair
/// emitted by the parser's classic-credit producer.
///
/// Returns `None` on malformed inputs:
/// * `issuer` doesn't parse as a `G...` StrKey,
/// * `code` length is zero or > 12,
/// * code is 4 bytes-or-shorter → `CreditAlphanum4`; 5-12 bytes →
///   `CreditAlphanum12`; trailing-NUL padding handled by `AssetCode*`
///   fixed-array layout.
///
/// Errors are returned silently (`None`) so callers can `warn!`-log and
/// skip — never failing the parse / persist pipeline.
fn build_credit_asset(code: &str, issuer: &str) -> Option<Asset> {
    let issuer = AccountId::from_str(issuer).ok()?;
    let bytes = code.as_bytes();
...
/// Forward-derive `is_sac=true` overrides from observed classic assets.
///
/// For each `ExtractedAsset` whose `asset_type` is `Native` or
/// `ClassicCredit`, derive the deterministic SAC contract_id from the
/// asset's `ContractIdPreimage::Asset(_)` envelope (per stellar-core,
/// see [`derive_sac_contract_id`]) and emit a [`SacOverride`].
///
/// The override exists to flip `soroban_contracts.is_sac=true` on rows
/// that pre-date the in-window create_contract op (cross-batch
/// boundary; the SAC was deployed in a ledger we did not index in the
/// current backfill window). Without this override, a SAC referenced
/// only via classic asset rows would persist as `is_sac=false` and
/// downstream NFT/Fungible filters would route it incorrectly.
///
/// Behaviour:
/// * `Native` → `ContractIdPreimage::Asset(Asset::Native)` → SAC `C...`.
/// * `ClassicCredit` → `(code, issuer)` rebuilt into XDR
///   `Asset::CreditAlphanum4`/`12` then derived.
/// * `Sac` / `Soroban` → skipped (already classified at deploy time, or
///   bespoke contract tokens with no SAC mapping).
/// * Missing `asset_code` / `issuer_address` on a `ClassicCredit`,
///   malformed issuer StrKey, asset code too long, or XDR serialize
///   failure → `warn!`-logged and skipped. Never fails the caller.
///
/// Output order matches input order — callers using positional binds
/// (UNNEST on PG, RowBinary on CH) get a stable index across rebuilds.
pub fn derive_sac_overrides_from_assets(
    assets: &[ExtractedAsset],
    passphrase: &str,
) -> Vec<SacOverride> {
    let net_id = network_id(passphrase);
    let mut out: Vec<SacOverride> = Vec::with_capacity(assets.len());
    for asset in assets {
        let xdr_asset = match asset.asset_type {
            TokenAssetType::Native => Asset::Native,
            TokenAssetType::ClassicCredit => {
                let Some(code) = asset.asset_code.as_deref() else {
                    tracing::warn!(
                        contract_id = asset.contract_id.as_deref().unwrap_or(""),
                        "SAC override: classic_credit asset missing asset_code; skipped"
                    );
                    continue;
                };
                let Some(issuer) = asset.issuer_address.as_deref() else {
                    tracing::warn!(
                        contract_id = asset.contract_id.as_deref().unwrap_or(""),
                        "SAC override: classic_credit asset missing issuer_address; skipped"
                    );
                    continue;
                };
                let Some(built) = build_credit_asset(code, issuer) else {
                    tracing::warn!(
                        code,
                        issuer,
                        "SAC override: invalid classic_credit asset shape; skipped"
                    );
                    continue;
                };
                built
            }
            TokenAssetType::Sac | TokenAssetType::Soroban => continue,
        };
        let identity = asset_to_identity(&xdr_asset);
        let preimage = ContractIdPreimage::Asset(xdr_asset);
        match derive_sac_contract_id(&preimage, &net_id) {
            Ok(contract_id) => out.push(SacOverride {
                contract_id,
            ...
            Err(e) => tracing::warn!(
                error = %e.message,
                "SAC override: derive_sac_contract_id failed; skipped"
            ),
        }
    }
    out
...
mod tests {
    ...
    // -- SacOverride derivation from ExtractedAsset (task 0218 / 0220 shared) --

    fn synthetic_native_asset() -> ExtractedAsset {
        ExtractedAsset {
            asset_type: TokenAssetType::Native,
            asset_code: None,
            issuer_address: None,
            contract_id: None,
            name: Some("Stellar Lumen".into()),
            total_supply: None,
            holder_count: None,
        }
    }

    fn synthetic_usdc_credit() -> ExtractedAsset {
        ExtractedAsset {
            asset_type: TokenAssetType::ClassicCredit,
            asset_code: Some("USDC".into()),
            issuer_address: Some(
    ...
    fn overrides_native_xlm_maps_to_mainnet_sac() {
        let overrides =
            derive_sac_overrides_from_assets(&[synthetic_native_asset()], MAINNET_PASSPHRASE);
        assert_eq!(overrides.len(), 1);
        assert_eq!(
            overrides[0].contract_id, "CAS3J7GYLGXMF6TDJBBYYSE3HQ6BBSMLNUQ34T6TZMYMW2EVH34XOWMA"
        );
        assert_eq!(overrides[0].identity, SacAssetIdentity::Native);
    }
    ...
    fn overrides_usdc_classic_credit_maps_to_mainnet_sac() {
        let overrides =
            derive_sac_overrides_from_assets(&[synthetic_usdc_credit()], MAINNET_PASSPHRASE);
        assert_eq!(overrides.len(), 1);
        assert_eq!(
            overrides[0].contract_id, "CCW67TSZV3SSS2HXMBQ5JFGCKJNXKZM7UQUWUZPUTHXSTZLEO7SJMI75"
        );
        match &overrides[0].identity {
            SacAssetIdentity::Credit { code, issuer } => {
                assert_eq!(code, "USDC");
                assert_eq!(issuer, "GA5ZSEJYB37JRC5AVCIA5MOP4RHTM335X2KGX3IHOJAPP5RE34K4KZVN");
            }
            other => panic!("expected Credit override, got {other:?}"),
        }
    }

    /// 5–12-byte codes route through `Asset::CreditAlphanum12`. The
    /// contract_id must equal a direct derivation with the same XDR
    /// shape — verifies the AlphaNum12 path matches stellar-core's
    /// envelope layout.
    #[test]
    fn overrides_alphanum12_code_uses_credit_alphanum12_path() {
        let asset = ExtractedAsset {
            asset_type: TokenAssetType::ClassicCredit,
            asset_code: Some("LONGCODE12".into()), // 10 bytes → AlphaNum12
            issuer_address: Some(
                "GA5ZSEJYB37JRC5AVCIA5MOP4RHTM335X2KGX3IHOJAPP5RE34K4KZVN".into(),
            ),
            contract_id: None,
        ...
        let overrides = derive_sac_overrides_from_assets(&[asset], MAINNET_PASSPHRASE);
        assert_eq!(overrides.len(), 1);
        // Cross-check via direct XDR construction.
        let issuer = AccountId::from_str(
            "GA5ZSEJYB37JRC5AVCIA5MOP4RHTM335X2KGX3IHOJAPP5RE34K4KZVN",
        )
        .unwrap();
        let mut padded = [0u8; 12];
        padded[..10].copy_from_slice(b"LONGCODE12");
        let xdr_asset = Asset::CreditAlphanum12(AlphaNum12 {
            asset_code: AssetCode12(padded),
            issuer,
        });
        let direct = derive_sac_contract_id(
            &ContractIdPreimage::Asset(xdr_asset),
            &network_id(MAINNET_PASSPHRASE),
        )
        .unwrap();
        assert_eq!(overrides[0].contract_id, direct);
    ...
    fn overrides_sac_and_soroban_assets_are_skipped() {
        let assets = [
            ...
            ExtractedAsset {
                ...
                asset_code: None,
                issuer_address: None,
                contract_id: Some("CDEFGHIJ".into()),
                name: None,
                total_supply: None,
                holder_count: None,
            },
        ...
        let overrides = derive_sac_overrides_from_assets(&assets, MAINNET_PASSPHRASE);
        assert!(overrides.is_empty(), "Sac/Soroban variants must be skipped");
    }
    ...
    fn overrides_classic_credit_with_invalid_issuer_strkey_is_skipped() {
        let asset = ExtractedAsset {
            asset_type: TokenAssetType::ClassicCredit,
            asset_code: Some("USDC".into()),
            issuer_address: Some("NOT_A_VALID_STRKEY".into()),
            contract_id: None,
            name: None,
            total_supply: None,
            holder_count: None,
    ...
    fn overrides_classic_credit_missing_code_or_issuer_is_skipped() {
        ...
        let overrides =
            derive_sac_overrides_from_assets(&[no_code, no_issuer], MAINNET_PASSPHRASE);
        assert!(overrides.is_empty());
    }

    #[test]
    fn overrides_emits_one_per_observed_asset_in_input_order() {
        let assets = [synthetic_native_asset(), synthetic_usdc_credit()];
        let overrides = derive_sac_overrides_from_assets(&assets, MAINNET_PASSPHRASE);
        assert_eq!(overrides.len(), 2);
        assert_eq!(
            overrides[0].contract_id, "CAS3J7GYLGXMF6TDJBBYYSE3HQ6BBSMLNUQ34T6TZMYMW2EVH34XOWMA",
            "Native must be first (matches input order)"
        );
        assert_eq!(
            overrides[1].contract_id, "CCW67TSZV3SSS2HXMBQ5JFGCKJNXKZM7UQUWUZPUTHXSTZLEO7SJMI75",
            "USDC ClassicCredit must be second"
        );
    }
}
