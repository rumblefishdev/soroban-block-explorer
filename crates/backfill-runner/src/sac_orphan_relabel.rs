...
//! P23, CAP-67 unified asset events. A 2026-06-18 full cryptographic split
//! proved every orphan in this population (5,607/5,607) is such a SAC. The live
//! parser fix (`xdr_parser::derive_sac_overrides_from_events`) labels them going
//! forward; this pass fixes the EXISTING history so the `nft-reclassify` step
//! (task 0303) DROPS their ~51.5M false-positive `nfts_pending` rows (the i128
//! transfer amount mis-read as a token_id) instead of churning.
//!
//! ## Mechanism
//!
...
/// `(orphan strkey, one sample event's topics JSON)` from the orphan×events
/// join. `topics_xdr` is `serde_json::to_string(topics)` despite the name
/// (`stage.rs:866`).
...
/// One sample SAC-control event's topics per orphan. `any()` collapses to a
/// single event per contract; the `signature` filter bounds the events scanned
/// (avoid an unfiltered `soroban_events` scan).
async fn fetch_orphan_events(
    client: &ClickhouseClient,
) -> Result<Vec<OrphanEventRow>, BackfillError> {
    let sql = format!(
        "SELECT sc.contract_id AS contract_id, any(e.topics_xdr) AS topics_xdr \
         FROM soroban_contracts AS sc FINAL \
         INNER JOIN soroban_events AS e ON e.contract_id = sc.id \
