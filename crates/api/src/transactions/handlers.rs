...
/// `transaction_hash_index` (a hash prefix, so more than one only when two
/// hashes share it), and the first whose `transactions` row carries the full
/// hash wins. No candidate, or none that matches, is `Ok(None)` → 404.
