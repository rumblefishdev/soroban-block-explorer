...
///
/// Transport-error judgement is delegated to
/// [`crate::http_transient::is_transient_reqwest`] — the single rule shared
/// with the SEP-1 path. Default is **permanent** — only the variants below
/// escape to retry.
pub fn is_transient(err: &NftTokenUriError) -> bool {
    match err {
        NftTokenUriError::Http { source, .. } => {
            crate::http_transient::is_transient_reqwest(source)
        }
